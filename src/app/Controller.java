        ...
        if (mode.toUpperCase().equals("M")){
            ...
            for(int i=0;i<rows;i++){
                for(int j=0; j<cols; j++){
                    ...
                    String[] inp = sc.nextLine().split("\\s*,\\s*");
                    u1[i][j] = inp[0];
                    u2[i][j] = inp[1];
...
//        Controller.printFormattedMatrix(u1, u2);
    ...
    public static void displayNashEq(String[][] u1, String[][] u2){
        ...
        for(int j=0; j<u1[0].length; j++){
            ...
            for(int index: indices) u1Marked[index][j] = "H";
        }
        ...
        for(int i=0; i<u2.length; i++){
            ...
            for(int index: indices) u2Marked[i][index] = "H";
        }
        ...
        if(nashEq.equals("")){
            System.out.println("There are no Pure Nash Equilibrium.");
        } else {
    ...
    public static void displayPayoffRandomBeliefs(String[][] u1, String[][] u2){
        ...
        for(int i=0;i<p1payoffs.length;i++){
            ...
            else if (p1BestPayOff==p1payoffs[i]) p1BestResponseIndices.add(i);
        }
        
        String p1BestResponses = "{";
        for(int i: p1BestResponseIndices) p1BestResponses += "A" + (i+1) + ", ";
        ...
        for(int i=0;i<p2payoffs.length;i++){
            if(p2payoffs[i]>p2BestPayOff){
                ...
                p2BestPayOff = p1payoffs[i];
            }
            else if (p2BestPayOff==p2payoffs[i]) p2BestResponseIndices.add(i);
        }
        
        String p2BestResponses = "{";
        for(int i: p2BestResponseIndices) p2BestResponses += "B" + (i+1) + ", ";
    ...
    private static void printStrategiesAndPayoffs(String[][] u, int n){
        ...
        String strategies = "{";
        if(n==1){
            for(int i=1; i<=u.length; i++) strategies = strategies + "A" + i + ", ";
        } else {
            for(int i=1; i<=u[0].length; i++) strategies = strategies + "B" + i + ", ";
        }
    ...
    private static String getPaddedString(String str, int totalLength, char paddingChar) {
        ...
        for (int i = 0; i < leftPadding; i++) sb.append(paddingChar);
        sb.append(str);
        for (int i = 0; i < rightPadding; i++) sb.append(paddingChar);

        return sb.toString();
    }
//    private static void printFormattedMatrix(String[][] u1, String[][] u2){
//        int rows = u1.length;
//        int cols = u2[0].length;
//
//        // Compute the width of each cell (max width needed)
//        int cellWidth = 0;
//        for (int i = 0; i < rows; i++) {
//            for (int j = 0; j < cols; j++) {
//                String s = String.format("(%s,%s)", u1[i][j], u2[i][j]);
//                if (s.length() > cellWidth) cellWidth = s.length();
//            }
//        }
//        cellWidth += 2; // add extra padding for spacing
//        
//
//        // Print Player 2 headers
